...
if ($request->hasFile('document')) {  
    ...
    try{
      ...
      // Step 3: Set the file metadata (such as name and parent folder ID)
...
// try{  
        ...
        if($officeHead == null) //if the user are from dept n service but not officehead
        {
            ...
            $Request_notesheet->approverLevel = $request->approverLevel;
            $Request_notesheet->document = $filePath;  //stor file path
            $Request_notesheet->save(); 
           
    
            // dd($supervisorEmail->emailId);
            Mail::to($supervisorEmail->emailId)  // to this address
            ->send(new MyTestMail($supervisor)); //content this
...
// try{
    ...
    if($request->status == "Recommended" ){  //&& $request->remarks != ''
        ...
        ->with('success','You have recommended and forwarded the Notesheet');
                ...
                return redirect('home')
                ->with('page','notesheetReview')
                ->with('success','You have Approved the Notesheet');   
            }
                ...
                if($request->status2 == "Rejected"){
                    ...
                    return redirect('home')
                    ->with('page','notesheetReview')
                    ->with('error','You have rejected the Notesheet');    
                }    
                
                else{
                    return redirect('home')
                    ->with('page','notesheetReview')
                    ->with('error','You cannot leave the remarks field empty!!');  
                }
