...
<input type="hidden" class="form-control" value="{{ Auth::user()->empId }}" name="empId" id="empId">
        ...
        <div class="reviewTitle card-header bg-green text-center">
            <h5><b>General Manager Review (HR Services)</b></h5>
        </div>
...
<script>
    $(document).ready(function() {
        function loadDataTable(service = '', office = '') {
            $('#reviewPage').DataTable({
                ...
                destroy: true,
                ajax: {
                    url: "{{ route('HR_Services_GM.index') }}",
                    data: {
                        serviceType: service
                    }
                },
    ...
    // Disable the approve button for that row
    $(`.approve-btn[data-id="${rowId}"]`).prop('disabled', true);
    $(this).prop('disabled', true).text('Processing...');
    
    $.ajax({
        url: "{{ route('HR_Services_GM.GM_hrservice') }}", 
        method: 'POST',
        data: {
            id: rowId,  
            empId: empId,          
