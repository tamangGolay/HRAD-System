...
<div class="container">
    <a class="btn success" href="javascript:void(0)" id="manageDesignation">Add Designation Name&nbsp;&nbsp;<i class="fa fa-plus" aria-hidden="true"> </i></a>
                ...
                <input type="hidden"  value="{{ csrf_token() }}">
                   ...
                   <input type="hidden" name="id" id="designation_id">
                    ...
                    <div class="col-sm-offset-2 col-sm-10 text-center">
                     <button type="submit"  class="btn btn-outline-success" id="designationButton" value="create">Save changes
                     </button>
...
<script type="text/javascript">
    ...
    var table = $('.data-table').DataTable({
        ...
        columns: [
            {data: 'id', name: 'id'},
            {data: 'desisNameShort', name: 'desisNameShort'},
            {data: 'desisNameLong', name: 'desisNameLong'},
            {data: 'action', name: 'action', orderable: true, searchable: true},
        ]
    });

    //After Clicking the Add New button it will trigger here
    $('#manageDesignation').click(function () {
        $('#designationButton').val("create-room");
        $('#designation_id').val('');
        $('#Form').trigger("reset");
  ...
  //  After clicking the edit button it will trigger here
    $('body').on('click', '.edit', function () {
      ...
      $.get("{{ route('designation.index') }}" +'/' + designation_id +'/edit', function (data) {
          $('#modelHeading').html("Edit Designation details");
          $('#designationButton').val("edit-room");
          $('#ajaxModel').modal('show');
          $('meta[name="csrf-token"]').attr('content'),
          $('#designation_id').val(data.id);
          $('#desisNameShort').val(data.desisNameShort); //input id,database
          $('#desisNameLong').val(data.desisNameLong);
      })
   });
    ...
    $('#designationButton').click(function (e) {
        ...
        table.draw();
          ...
          error: function (data) {
              ...
              $('#designationButton').html('Save Changes');
