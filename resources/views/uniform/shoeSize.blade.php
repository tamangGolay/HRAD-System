...
<div class="container">
    <a class="btn success" href="javascript:void(0)" id="manageShoesize">Add Shoe Size&nbsp;&nbsp;<i class="fa fa-plus" aria-hidden="true"> </i></a>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <input type="hidden"  value="{{ csrf_token() }}">
                   ...
                   <input type="hidden" name="id" id="shoesize_id">
                    ...
                    <div class="col-sm-offset-2 col-sm-10 text-center">
                     <button type="submit"  class="btn btn-outline-success" id="shoesizeButton" value="create">Save changes
                     </button>
...
<div class="modal fade" id="shoesizeModel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="shoesizeHeading"></h4>
            </div>
            <div class="modal-body">
                <form id="Form" name="Form" class="form-horizontal">
                @csrf
                ...
                <div class="col text-center col-form-label col-md-center col-sm-2 col-md-10 col-lg-12">
                    <button type="submit" class="btn btn-outline-success" id="shoesizeDeleteButton" value="create">Yes</button>
...
<script type="text/javascript">
    ...
    var table = $('.data-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('shoesize.index') }}",
        columns: [
            {data: 'id', name: 'id'},
            {data: 'usShoeSize', name: 'usShoeSize'},  
            {data: 'ukShoeSize', name: 'ukShoeSize'},
    ...
    $('#manageShoesize').click(function () {
        $('#shoesizeButton').val("create-room");
        $('#shoesize_id').val('');
        $('#Form').trigger("reset");
        $('#modelHeading').html("Add new Shoe Size");
  ...
  //  After clicking the edit button it will trigger here
    $('body').on('click', '.edit', function () {
      var shoesize_id = $(this).data('id');
     
      $.get("{{ route('shoesize.index') }}" +'/' + shoesize_id +'/edit', function (data) {
          $('#modelHeading').html("Edit Shoe size details");
          $('#shoesizeButton').val("edit-room");
          $('#ajaxModel').modal('show');
          $('meta[name="csrf-token"]').attr('content'),
          $('#shoesize_id').val(data.id);
          $('#usShoeSize').val(data.usShoeSize); //input id,database
          $('#ukShoeSize').val(data.ukShoeSize);
...
//   After clicking save changes in Add and Edit it will trigger here
    ...
    $('#shoesizeButton').click(function (e) {
        ...
        $.get('/getView?v=shoesize',function(data){        
           $('#contentpage').empty();                          
        ...
        table.draw();
          ...
          error: function (data) {
              ...
              $('#shoesizeButton').html('Save Changes');
  ...
  //  After clicking delete it will trigger here
    ...
    $('body').on('click', '.deleteShoesize', function () {
      var shoesize_id = $(this).data('id');
     
      $.get("{{ route('shoesize.index') }}" +'/' + shoesize_id +'/edit', function (data) {
          $('#shoesizeHeading').html("Do you want to delete?");
          $('#shoesizeDeleteButton').val("edit-room");
          $('#shoesizeModel').modal('show');
          $('meta[name="csrf-token"]').attr('content'),
          $('#shoesize_id').val(data.id);
          $('#usShoeSize').val(data.usShoeSize); //input id,database
          $('#ukShoeSize').val(data.ukShoeSize);
  ...
  // after clicking yes in delete
    $('#shoesizeDeleteButton').click(function (e) {
        e.preventDefault();
        $(this).html('Deleting...');
    
        $.ajax({
          data: $('#Form').serialize(),
          url: "{{ route('destroyshoesize') }}",
          type: "POST",
          dataType: 'json',
          success: function (data) {
     
              $('#Form').trigger("reset");
              $('#shoesizeModel').modal('hide');
              table.draw();
              window.onload = callajaxOnPageLoad(page);
        var alt = document.createElement("div");
             alt.setAttribute("style","position:absolute;top:20%;left:50%;background-color:#BFC9CA;border-color:#34495E;");
         ...
         $.get('/getView?v=shoesize',function(data){        
          ...
          error: function (data) {
              console.log('Error:', data);
              $('#shoesizeDeleteButton').html('Save Changes');
          }
      });
    });
