<div class="row ">
	<div class="col">
		<div class="card ">
			...
			<form method="POST" action="/vehicleapprove" enctype="multipart/form-data" accept-charset="UTF-8"> @csrf
				...
				<div class="card-body table-responsive p-0">
					<table id="table1" class="table table-hover table-striped table-bordered">
						...
						<tbody> @foreach($cmdpoReview as $rv)
							<tr>
								<td> {{$rv->id}} </td>
								<td> {{$rv->emp_id}} </td>
								...
								<td>
									...
									<button type="submit" name="id[]" id="id" value="{{$rv->id}}" class="button1 btn btn-outline-success text-dark"> Approve</button> &nbsp;&nbsp;
									</form> </td>
			...
			<div class="float-right"> {{$cmdpoReview->links()}} </div>
			<div>
				<!--/card-body-->
				</form>
			</div>
		...
		$(document).ready(function() {
			document.getElementById('contenthead').innerHTML = '<Strong d-flex justify-content center>CMDPO Review Page</strong>';
		});
		</script>
		<!-- jquery-validation -->
