	...
	<div class="col">
		<div class="card ">
			...
			<form method="POST" action="/vehiclereject" enctype="multipart/form-data" accept-charset="UTF-8"> @csrf
				...
				<input type="hidden" name="token" id="tokenid" value="{{ csrf_token()}}">
				<button type="submit" name="id[]" id="id" value="{{$rv->id}}" class=" button2 btn btn-outline-danger text-dark text-center"> Reject
					</button>
				<br>
				<br>
				<div>
			...
			<div class="float-right"> {{$DcdReview->links()}} </div>
			<div>
				<!--/card-body-->
				</form>
			</div>
