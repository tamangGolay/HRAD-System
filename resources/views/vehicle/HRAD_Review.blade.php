		...
		<div class="card ">
			...
			<form method="POST" action="/vehicleapprove" enctype="multipart/form-data" accept-charset="UTF-8"> @csrf
				...
				<div class="card-body table-responsive p-0">
					<table id="table12" class="table table-hover table-striped table-bordered">
						<thead>
						...
						<tbody> @foreach($HradReview as $rv)
							<tr>
								<td> {{$rv->id}} </td>
								<td> {{$rv->emp_id}} </td>
			...
			<form method="POST" action="/vehiclereject" enctype="multipart/form-data" accept-charset="UTF-8"> @csrf
				<input type="hidden" name="token" id="tokenid" value="{{ csrf_token()}}">
				<button type="submit" name="id[]" id="id" value="{{$rv->id}}" class=" button2 btn btn-outline-danger text-dark text-center"> Reject</button>
				<br>
				<br>
				<div>
			...
			<div class="float-right"> {{$HradReview->links()}} </div>
			<div>
				<!--/card-body-->
				</form>
			</div>
