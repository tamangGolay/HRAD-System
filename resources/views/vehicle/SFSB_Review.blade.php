		...
		<div class="card ">
			...
			<form method="POST" action="/vehicleapprove" enctype="multipart/form-data" accept-charset="UTF-8"> @csrf
				...
				<div class="card-body table-responsive p-0">
					<table id="table16" class="table table-hover table-striped table-bordered">
						<thead>
						...
						<tbody> @foreach($sfsbReview as $rv)
							<tr>
								<td> {{$rv->id}} </td>
								<td> {{$rv->emp_id}} </td>
								...
								<td>
									...
									<button type="submit" name="id[]" id="id" value="{{$rv->id}}" class=" button1 btn btn-outline-success text-center text-dark"> Approve</button> &nbsp;&nbsp; </td>
								<td>
			</form>
			<form method="POST" action="/vehiclereject" enctype="multipart/form-data" accept-charset="UTF-8"> @csrf
				<input type="hidden" name="token" id="tokenid" value="{{ csrf_token()}}">

				<button type="submit" name="id[]" id="id" value="{{$rv->id}}" class=" button2 btn btn-outline-danger text-center text-dark ">Reject</button>
			...
			</td>s </tr> @endforeach </tbody>
			</table>
			<div class="float-right"> {{$sfsbReview->links()}} </div>
			<div>
				<!--/card-body-->
				</form>
			</div>
