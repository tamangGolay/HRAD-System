		...
		<div class="card ">
			...
			<form method="POST" action="/vehicleapprove" enctype="multipart/form-data" accept-charset="UTF-8"> @csrf
				...
				<div class="card-body table-responsive p-0">
					<table id="table17" class="table table-hover table-striped table-bordered">
						<thead>
						...
						<tbody> @foreach($spbdReview as $rv)
							<tr>
								<td> {{$rv->id}} </td>
								<td> {{$rv->emp_id}} </td>
								...
								<td style="width:40%">
									...
									<button type="submit" name="id[]" id="id" value="{{$rv->id}}" class=" button1 btn btn-outline-success text-dark text-center"> Approve</button> &nbsp;&nbsp; </td>
								<td>
			</form>
			...
			<div class="float-right"> {{$spbdReview->links()}} </div>
			<div>
				<!--/card-body-->
				</form>
			</div>
